...
# --------------------------------------------------------------------------
# Real-image detection (not just Content-Type)
# --------------------------------------------------------------------------
...
async def test_update_avatar_rejects_polyglot_appended_payload():
    ...
    # Stored file must be a clean re-encoded PNG, not contain the payload.
...
# --------------------------------------------------------------------------
# Size / dimension limits (resource-consumption guard)
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Filename unguessability (IDOR / broken-access-control guard)
# --------------------------------------------------------------------------
...
def test_generate_avatar_token_is_random_and_unguessable():
    ...
    assert len(tokens) == 50  # no collisions across 50 draws
    for token in tokens:
...
async def test_update_avatar_deletes_old_avatar_after_success(isolated_avatar_dir):
    ...
    # Simulate an existing stored avatar from a prior upload.
    old_token = "a" * 32
    ...
    assert len(remaining) == 1  # only the newly stored file
...
async def test_update_avatar_cleans_up_new_file_if_db_commit_fails(isolated_avatar_dir):
    ...
    # No orphaned file should remain if the DB write failed.
    assert list(isolated_avatar_dir.glob("*")) == []


# --------------------------------------------------------------------------
# Download controller: path traversal / arbitrary filename rejection
# --------------------------------------------------------------------------
...
def test_username_schema_rejects_whitespace_only_padding_tricks():
    ...
    with pytest.raises(Exception):
        UpdateProfileParams(token="t", username="ab cd")  # embedded space not allowed
